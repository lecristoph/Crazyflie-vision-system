...
def getCFparams(scf, available):
    try:
        vbat, blevel, pwm, temp = np.asarray(getData()).tolist()
    except TypeError:
...
if __name__ == '__main__':
    ...
    figname = "camera_exp.png"
    # Title of plot
    figtitle = "Battery level"
    # Name of dumpfile
    dumpfile_name = "batteryleveltest_withcamera_on.yaml"
    ...
    if len(available) == 0:
        ...
        with SyncCrazyflie(URI) as scf:
            ...
            ar_blevel = np.append(ar_blevel, cf2_blevel)
            ar_pwm = np.append(ar_pwm, cf2_pwm)
            ar_temp = np.append(ar_temp, cf2_temp)
            ...
            while time.time() < endTime and cf2_bat >= low_battery and A:
                ...
                try:
                    if yaw_cmd is not None:
                        ...
                        cf2_bat, cf2_blevel, cf2_pwm, cf2_temp = cf2.update(-cval_psi,
                                                      'psi', turnRate)
...
if len(available) != 0:
    ...
    data = {'timestamp': np.asarray(ar_time).tolist(),
            'voltage_level': np.asarray(ar_bat).tolist(),
            'battery_level': np.asarray(ar_blevel).tolist(),
            'temperature': np.asarray(ar_temp).tolist(),
            'pwm': np.asarray(ar_pwm).tolist(),
